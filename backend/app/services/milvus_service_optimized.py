...
class MilvusServiceOptimized:
    """优化的Milvus服务类 - 支持连接池、批量操作、异步加载"""
...
# 全局Milvus实例（延迟初始化）
_milvus_service_opt: Optional[MilvusServiceOptimized] = None

def get_milvus_service_optimized() -> MilvusServiceOptimized:
    """获取优化的Milvus服务实例（单例模式）"""
    global _milvus_service_opt
    if _milvus_service_opt is None:
        _milvus_service_opt = MilvusServiceOptimized()
